...
$(document).ready(function() {
    $(".nickname-button").click(function() {
        var $nicknameContainer = $(this).closest(".nickname-section");
        var $nicknameText = $nicknameContainer.find(".nickname-text"); 
        var $nicknameInput = $nicknameContainer.find(".nickname-input"); 
        var $button = $(this);
        
        if ($nicknameText.is(":visible")) {
            $nicknameText.hide();
            $nicknameInput.show();
            $nicknameInput.val($nicknameText.text());
            $button.text("저장 ");
            $nicknameContainer.find("h5").css("font-weight", "bold").css("opacity", 1);
        } else {
            var newNickname = $nicknameInput.val();
            $.post("/user/modifyNickname", { newNickname: newNickname }, function(response) {
                if (response === "success") {
                    $nicknameText.text(newNickname); 
                    $nicknameInput.hide();
                    $nicknameText.show();
                    $button.text("변경");
                    $nicknameContainer.find("h5").css("font-weight", "normal").css("opacity", 0.3);
                } else {
    ...
    $(".int-button").click(function() {
        var $intContainer = $(this).closest(".int-section");
        var $intText = $intContainer.find(".int-text");
        var $intInput = $intContainer.find(".int-input");
        var $button = $(this);
        ...
        if ($intText.is(":visible")) {
            ...
            $button.text("저장 ");
            $intContainer.find("h5").css("font-weight", "bold").css("opacity", 1);
        } else {
            var newInt = $intInput.val();
            $.post("/user/modifyInt", { newInt: newInt }, function(response) {
                if (response === "success") {
                    $intText.text(newInt);
                    $intInput.hide();
                    $intText.show();
                    $button.text("변경");
                    $intContainer.find("h5").css("font-weight", "normal").css("opacity", 0.3);
                } else {
                    alert("소개 변경에 실패했습니다.");
                }
            });
    ...
    <div class="user-info-content">
        ...
        <div class="user-info-details">
            <div class="user-info-names">
                <strong>${member.mem_id}</strong><br>
                <span>${member.mem_email}</span>
            </div>
            <div class="user-info-buttons">
                <button id="updateProfileButton">프로필 업데이트</button>
    			<button id="deleteProfileButton">프로필 삭제</button>
            </div>
        </div>
    </div>
</div>
