...
<script type="text/javascript">
	$(function(){
		...
		$('#btn_status1').click(function(){
			...
			$('#dialog').dialog('close');
			if(${way} == '1'){
				location.href='${pageContext.request.contextPath}/myPage/buying.do?way=1&status=1';
			}else if(${way} == '2'){
				location.href='${pageContext.request.contextPath}/myPage/buying.do?way=2&status=1';
			}
		});
		$('#btn_status2').click(function(){
			if(${way} == '1'){
				...
				location.href='${pageContext.request.contextPath}/myPage/buying.do?way=1&status=2';
			}else if(${way} == '2'){
				$('#status').attr('value','검수준비중').attr('data-status','2');	
				$('#dialog').dialog('close');
				location.href='${pageContext.request.contextPath}/myPage/buying.do?way=2&status=2';
			}
		});
		$('#btn_status3').click(function(){
			if(${way} == '1'){
				...
				$('#dialog').dialog('close');
				location.href='${pageContext.request.contextPath}/myPage/buying.do?way=1&status=3';
			}else if(${way} == '2'){
				$('#status').attr('value','검수중').attr('data-status','3');	
				$('#dialog').dialog('close');
				location.href='${pageContext.request.contextPath}/myPage/buying.do?way=2&status=3';
			}
		});
		...
		$('#btn_status4').click(function(){
			...
			$('#dialog').dialog('close');
			location.href='${pageContext.request.contextPath}/myPage/buying.do?way=2&status=4';
		});
		
		$('#btn_status5').click(function(){
			$('#status').attr('value','거래실패').attr('data-status','6');	
			$('#dialog').dialog('close');
			location.href='${pageContext.request.contextPath}/myPage/buying.do?way=2&status=6';
		});
...
<div class="table-header">
	<div class="title-list">
		<table>
			<thead>
				<tr>
					<td onclick="location.href='${pageConetext.request.contextPath}/myPage/buying.do?way=1'">
						<p>${bidCount}</p>
						<p>구매입찰</p>
					</td>
					<td onclick="location.href='${pageConetext.request.contextPath}/myPage/buying.do?way=2'">
						<p>${tradeCount}</p>
						<p>진행중</p>
					</td>
					<td onclick="location.href='${pageConetext.request.contextPath}/myPage/buying.do?way=3'">
						<p>${quitCount}</p>
						<p>종료</p>
					</td>
				</tr>
	  ...
	  <tbody>
	    <c:forEach items="${list}" var="list">
	      <tr class="list-text">
	        ...
	        <td>${list.purchase_regDate}</td>
	        <td>${list.purchase_deadline}</td>
	        </c:if>
	        <c:if test="${way == 2}">
