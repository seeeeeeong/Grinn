...
<script type="text/javascript">
	$(function(){
		...
		$('#btn_status1').click(function(){
			...
			$('#dialog').dialog('close');
			if(${way} == '1'){
				location.href='${pageContext.request.contextPath}/user/buying.do?way=1&status=1';
			}else if(${way} == '2'){
				location.href='${pageContext.request.contextPath}/user/buying.do?way=2&status=1';
			}
		});
		$('#btn_status2').click(function(){
			if(${way} == '1'){
				...
				location.href='${pageContext.request.contextPath}/user/buying.do?way=1&status=2';
			}else if(${way} == '2'){
				$('#status').attr('value','검수준비중').attr('data-status','2');	
				$('#dialog').dialog('close');
				location.href='${pageContext.request.contextPath}/user/buying.do?way=2&status=2';
			}
		});
		$('#btn_status3').click(function(){
			if(${way} == '1'){
				...
				$('#dialog').dialog('close');
				location.href='${pageContext.request.contextPath}/user/buying.do?way=1&status=3';
			}else if(${way} == '2'){
				$('#status').attr('value','검수중').attr('data-status','3');	
				$('#dialog').dialog('close');
				location.href='${pageContext.request.contextPath}/user/buying.do?way=2&status=3';
			}
		});
		...
		$('#btn_status4').click(function(){
			...
			$('#dialog').dialog('close');
			location.href='${pageContext.request.contextPath}/user/buying.do?way=2&status=4';
		});
		
		$('#btn_status5').click(function(){
			$('#status').attr('value','거래실패').attr('data-status','6');	
			$('#dialog').dialog('close');
			location.href='${pageContext.request.contextPath}/user/buying.do?way=2&status=6';
		});
...
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/ssk/buying.css">  
<jsp:useBean id="now" class="java.util.Date" /> 
<fmt:formatDate value="${now}" pattern="yyyy/MM/dd" var="nowDate" />  
<div class="page-main">
	<div class="myPurchase">
		<div class="myPurchase-title">
			<div class="myPurchase-title-content">
				<h2>구매 내역</h2>
			</div>
		</div>
		...
		<div class="myPurchase-detail">
			<div class="myPurchase-head">
				<div class="head-list">
					<c:if test="${way == 1}">
						<c:if test="${status == 1}">
			...
			<div class="myPurchase-main-content">
				<c:if test="${count == 0}">
					<c:if test="${way == 1}">
						<div class="myPurchase-empty">구매 입찰 내역이 없습니다.</div>
					</c:if>
					<c:if test="${way == 2}">
						<div class="myPurchase-empty">구매 거래 내역이 없습니다.</div>
					</c:if>
					<c:if test="${way == 3}">
						<div class="myPurchase-empty">종료된 구매 내역이 없습니다.</div>
					</c:if>
				</c:if>
				<c:if test="${count > 0}">
					<table>
					  ...
					  <tbody>
					    <c:forEach items="${list}" var="list">
					      <tr class="list-text"<c:if test="${way == 1 and now > list.purchase_deadline}">style="color:red;"</c:if>>
					        ...
					        <td class="list-date">${list.purchase_deadline}</td>
					        <c:if test="${status == 3}">
					        <td>
					        	<input type="button" class="btn-deleteBid" onclick="location.href='${pageContext.request.contextPath}/purchase/deleteBid.do?purchase_num=${list.purchase_num}'">
					        </td>
					        </c:if>
					        </c:if>
					        <c:if test="${way == 2}">
